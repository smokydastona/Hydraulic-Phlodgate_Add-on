...
### Finding trinkets in the world

Trinkets are not craft-only. The first time any player opens a naturally generated container, a bounded
percentage roll may seed one catalog trinket into a free slot. The roll happens against the container's
captured snapshot, so every player who opens that container has the same chance to find it rather than the
first arrival taking it. Operators control both the toggle and the drop chance (0-100%) in World Settings.

## Instanced chest loot (Lootr-style, standalone-native)

Naturally generated containers are instanced per player. The first player to open one has its generated loot
captured, and every player afterwards works against their own copy, so nobody can empty a structure chest for
everyone else. Containers a player placed are recorded and permanently excluded, so ordinary storage behaves
normally. Stored items keep their count, custom name, lore, durability damage, and enchantments.

The store is bounded (192 containers, 28000 serialized characters, least-recently-used eviction) because world
dynamic properties are size-limited, and every persisted payload is re-sanitized on read so a corrupted
property degrades to "untracked" rather than throwing. The feature is an operator toggle.

This is not a port of Lootr's Java implementation. Bedrock's Script API exposes no custom container screen, so
the pack swaps the real container's contents at open time instead of rendering a per-player inventory view.
...
instance of that mob in the world. Every species ships its own **editable model** at
`RP*/models/entity/phlodgate/companion_<species>.geo.json` under `geometry.phlodgate.companion_<species>`,
vendored from Mojang's official `bedrock-samples` pack by `scripts/vendor-companion-models.mjs`. Open any of
those files directly in Blockbench to reshape a pet; bone names are preserved, so the reused vanilla
animations and render controllers keep binding. Every species also uses its own deterministic, original
hydraulic-circuit texture under `textures/entity/phlodgate/`, generated by
